    ...
    def get_offsets(self):
        ...
        for n_bit in range(len(self.data)-26):          #for each bit in data
            ...
            if syndrome in self.syndromes:              #if valid syndrome
                ...
                print self.offsets[indx], n_bit - last_bit, n_bit
                self.blocks.append([self.offsets[indx], n_bit - last_bit, n_bit])
                last_bit = n_bit

    def get_groups(self):
        for n in range(len(self.offset) - 4):
            if nmp.array_equal(self.offset[n:n+4], self.offsets):
                print n, self.offset[n:n+4]
...
print(len(data.sym),len(data.sym2))
x1 = 459*16
#x1 = 631*16
x2 = x1 + 16*5
plt.figure()
plt.plot(nmp.arange(x1,x2),data.bb2[x1:x2],'b.')
plt.plot(nmp.arange(x1,x2),data.phz[x1:x2],'r+')
plt.plot(nmp.arange(x1,x2),data.clk[x1:x2],'g')
#plt.plot(1000**data.crr[x1:x2:147])
#plt.psd(data.bb2,NFFT=2048,Fs=F_SAMPLE/DEC_RATE)
plt.figure()
#plt.plot((data.sym[x1/sps:x2/sps]-0.5)*2,'o')
plt.plot(data.sym2[x1/sps:x2/sps],'o')
plt.plot(data.sym[x1/sps:x2/sps],'*')
